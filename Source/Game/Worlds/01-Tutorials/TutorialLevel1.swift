...
class TutorialLevel1: TutorialLevel {
    ...
    func beginWave1(nextStep: @escaping NextStepBlock) {
        ...
        timeline.every(1.5...2.5, start: .Delayed(), times: 20,
            block: self.generateEnemy(wave1)) ~~> nextStep()
        timeline.every(1.5...4, start: .Delayed(), times: 13,
            block: self.generateEnemy(wave2)) ~~> nextStep()
    }
    ...
    func beginWave2(nextStep: @escaping NextStepBlock) {
        ...
        timeline.every(3...4, start: .Delayed(), times: 6,
            block: generateEnemy(wave1)) ~~> nextStep()
        timeline.every(2...3, start: .Delayed(5), times: 6,
            block: generateEnemy(wave2)) ~~> nextStep()
        timeline.every(2, start: .Delayed(10), times: 6,
            block: generateEnemy(wave3)) ~~> nextStep()
        timeline.every(1, start: .Delayed(21), times: 8,
    ...
    func beginWave4(nextStep: @escaping NextStepBlock) {
        ...
        timeline.every(4...6, start: .Delayed(), times: 5,
            block: generateLeaderEnemy(wave_leader)) ~~> nextStep()
        timeline.every(1.5...3.0, start: .Delayed(1), times: 16,
            block: generateEnemy(wave_soldier)) ~~> nextStep()
    }
